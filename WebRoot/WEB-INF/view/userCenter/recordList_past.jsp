			...
			$(".repayEarlyReward").each(function(index,item){
				$.post("${ctx}/user/repayEarlyReward",{id:$(item).attr("id")},function(data){
					if(data.indexOf("false")!=-1){
						$(item).html(data.substring(0,data.length-5));
						$(item).parent().parent().find(".user_cont_r_t_3c6").html("已还款")
						var html="<img src='${ctx }/images/user_touzi_01_h.png'/>";
						$(item).parent().parent().find(".user_cont_r_t_3c5").html(html);
					}else if (data.indexOf("true")!=-1){
						$(item).html(data.substring(0,data.length-4));
						var html="<img src='${ctx }/images/user_touzi_01_h.png'/>";
						$(item).parent().parent().find(".user_cont_r_t_3c5").html(html);
					}else{
						$(item).html(data.replace(/[a-z]*/g,""));
						var html="<a href='javascript:void(0)' onclick='repayEarly("+$(item).attr("id")+")'><img src='${ctx }/images/user_touzi_01.png'/></a>";
						$(item).parent().parent().find(".user_cont_r_t_3c5").html(html);
					}
				})
			});
				...
				<!-- 交易记录循环开始-->
					...
					<tr class="user_cont_r_t_3c">
						<td title="${record[1]}" class="user_cont_r_t_3c1">
							<a href="${ctx }/project/view?id=${record[0] }" target="_blank"><cc:subLength var="${record[1]}" length="8"/></a>
						</td>
						<td class="user_cont_r_t_3c2">${record[2]}</td>
						<td class="user_cont_r_t_3c3">${record[4]}</td>
						<td class="user_cont_r_t_3c4"><span id="${record[0] }" class="repayEarlyReward"></span></td>
						...
						<td class="user_cont_r_t_3c7">
							<a  class="theme-login2" href="javascript:loadlucreData('${record[1]}','${record[2]}','${record[4]}','${record[0]}')">
								<img src="${ctx }/images/user_touzi_02.png" />
							</a>
							<c:if test="${(record[6]==1||record[6]==2)&&record[8]!=null&&record[3]!=null}">
							<a href="${ctx }/user/createContract?id=${record[0] }" target="_blank">合同</a>
							</c:if>
						</td>
					</tr>
					</c:forEach>
